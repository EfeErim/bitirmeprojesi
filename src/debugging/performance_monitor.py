        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if len(values) >= 7:
                    ...
                    metrics.memory_used_gb = float(values[2]) / 1024  # MB to GB
                    metrics.memory_total_gb = float(values[3]) / 1024  # MB to GB
...
class MemoryProfiler:
    ...
    def __init__(self, leak_threshold: float = 0.05, check_interval: int = 100):
        self.leak_threshold = leak_threshold  # 5% growth per check
        ...
        self._snapshots: List[Tuple[float, float, float]] = []  # (time, allocated, reserved)
    ...
    def _detect_memory_leak(self, allocated: float, reserved: float) -> bool:
        ...
        growth_rate = (end_alloc - start_alloc) / time_diff  # GB/s
        
        # If memory is growing consistently without plateau
        if growth_rate > self.leak_threshold / 10:  # Convert to GB/s
            # Verify it's not just normal training fluctuation
            # Check if all recent snapshots show upward trend
            increasing = all(recent[i][1] <= recent[i+1][1] for i in range(len(recent)-1))
            if increasing and growth_rate > 0.01:  # At least 10MB/s growth
...
class DriveIOMonitor:
    ...
    def _collect_drive_metrics(self) -> DriveIOMetrics:
        """Collect Drive I/O metrics using iostat or similar."""
        ...
        try:
            ...
            if result.returncode == 0:
                # Parse iostat output for device statistics
                ...
                for line in lines:
                    if '/dev/sd' in line or '/dev/nvme' in line:
                        ...
                        if len(parts) >= 7:
                            ...
                            try:
                                metrics.read_speed_mb_s = float(parts[5])  # kB/s to MB/s would need conversion
                                metrics.write_speed_mb_s = float(parts[6])
                            except (ValueError, IndexError):
                                pass
                        break
        except (subprocess.CalledProcessError, FileNotFoundError, subprocess.TimeoutExpired):
            # Fallback: estimate based on file operations
            metrics = self._estimate_drive_metrics()
        
        return metrics
    
    def _estimate_drive_metrics(self) -> DriveIOMetrics:
        """Estimate Drive performance based on file operation timing."""
        metrics = DriveIOMetrics(timestamp=time.time())
        
        try:
            # Simple benchmark: write and read a small test file
...
def create_performance_monitor(config_path: Optional[str] = None) -> PerformanceMonitor:
    ...
    if config_path:
        try:
            ...
            config = config_manager.get('performance_monitoring', {})
        except Exception as e:
