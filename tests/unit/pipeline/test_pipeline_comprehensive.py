...
class TestIndependentMultiCropPipelineCache:
    ...
    @pytest.fixture
    def pipeline_with_cache(self, mock_pipeline_data):
        """Setup pipeline with mocked components using shared fixtures."""
        config = {'crops': mock_pipeline_data['crops'], 'cache_enabled': True}
        with patch('torch.cuda.is_available', return_value=False):
            pipeline = IndependentMultiCropPipeline(config, device='cpu')
            ...
            pipeline.router_analyzer.analyze.return_value = {
                'status': 'success',
                'classifications': [{
                    'species': mock_pipeline_data['crops'][0],
                    'confidence': mock_pipeline_data['mock_probabilities'][0, 0].item()
                }]
            }
            
            # Mock adapter using shared data
            ...
            pipeline.adapters[mock_pipeline_data['crops'][0]] = mock_adapter
