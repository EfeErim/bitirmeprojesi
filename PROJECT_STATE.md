...
- The taxonomy/prototype reconciler is now wired but config-gated. The next router-scalability step is to build full prototype artifacts, run `scripts/calibrate_router_prototype_reconciler.py` on the full held-out M2/router-eval surface, enable the reconciler only if the selected policy passes risk/coverage constraints, then rerun Notebook 8 M2 in Colab/GPU.
