...
- Implemented the first report-only taxonomy/prototype router slice: `src/router/taxonomy_registry.py`, `src/router/prototype_bank.py`, `scripts/build_taxonomy_registry.py`, and `scripts/build_router_prototype_bank.py` now build `taxonomy_registry.v1` plus `router_prototype_bank.v1` artifacts without changing Notebook 8 runtime behavior. The current local prototype backend is deterministic `image_stats_v1` scaffolding for artifact plumbing and calibration work, not a promoted BioCLIP/DINOv2 runtime backend.
...
- The taxonomy/prototype report-only scaffolding now exists; the next router-scalability step is a pure config-gated reconciler plus held-out calibration before wiring any reconciled crop/part into Notebook 8 adapter loading.
