...
- Exact M2 run path:
  ...
  3. Leave `M2_RUN_FULL_DEMO = True`, `M2_DEMO_LIMIT = None`, `M2_BATCH_SIZE = 12`, `M2_ADAPTER_BATCH_SIZE = 32`, and `M2_HANDOFF_CACHE = '.runtime_tmp/m2_router_prototype_handoff_cache.json'`; use the active full manifest configured in Notebook 8 for the current experiment.
  ...
  5. Run all cells. The single-image cell is skipped by default, and the final M2 cell runs all rows in the active configured full manifest.
...
The self-contained saved image package is under `docs/demo_assets/m2_full_image_set/`. The canonical stress manifest contains 602 asset-ready rows and is still the apples-to-apples comparison surface for existing M2 results:
...
For the final balanced experiment surface, use `docs/demo_assets/m2_full_image_set/manifests/m2_balanced_80_run_manifest.csv`. It keeps 80 rows for each supported crop/part target, keeps the existing 24 unsupported/non-plant safety probes, and adds external iNaturalist crop/part probes only where the 602-row set was under target:

`.\scripts\python.cmd scripts\run_demo_checklist.py --no-checklist --extra-manifest docs\demo_assets\m2_full_image_set\manifests\m2_balanced_80_run_manifest.csv --device cuda --adapter-root runs --batch-size 12 --adapter-batch-size 32 --handoff-cache .runtime_tmp\m2_router_prototype_handoff_cache.json`

The current Notebook 8 M2 batch/cache defaults live in `docs/notebook8_m2_run_state.json` and `docs/notebook8_m2_run_state.md`; do not rely on older hard-coded `12/32` examples. The runner persists router/prototype handoff outputs in `M2_HANDOFF_CACHE`, publishes that cache with each result folder, and restores the latest published cache when local `.runtime_tmp` is empty so repeated same-manifest runs can skip unchanged handoff work. The current prepared run keeps `M2_REFRESH_HANDOFF_CACHE = True`, uses the lower `6/12` batch sizes, and enables `M2_RUN_OPEN_WORLD_ROUTER_VALIDATION = True`. Before starting the long Colab/GPU run, execute `.\scripts\python.cmd scripts\validate_router_open_world_preflight.py --fail-on-invalid`.
...
- 2026-06-17 saved image package, updated 2026-06-24: 602 asset-ready rows are available in `docs/demo_assets/m2_full_image_set/manifests/m2_full_image_set_run_manifest.csv`.
...
- Saved package asset audit result: 602/602 rows are file-ready.
- 2026-06-28 balanced final-experiment manifest: `scripts/build_m2_balanced_demo_manifest.py --target-count 80` added 112 external iNaturalist rows and wrote `docs/demo_assets/m2_full_image_set/manifests/m2_balanced_80_run_manifest.csv`. The balanced asset audit passes 664/664.
